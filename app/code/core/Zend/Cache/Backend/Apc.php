...
class Zend_Cache_Backend_Apc extends Zend_Cache_Backend implements Zend_Cache_Backend_Interface
{
    ...
    private function recordTagUsage($tagId) {
		$old_tmaster_wrapper = apc_fetch(self::TAGS_MASTER_ID);
		...
		if (in_array( $tagId, $old_tmaster) ) {
			...
			return apc_store(self::TAGS_MASTER_ID, array($old_tmaster, time()));
		}
    }
	...
	private function removeTagUsage($tagId) {
		$tmaster_wrapper = apc_fetch(self::TAGS_MASTER_ID);
    ...
    public function clean($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        if ($mode==Zend_Cache::CLEANING_MODE_MATCHING_TAG) {
			...
			$idList = array();
			foreach ($tags as $tag) {
				$current_idList = apc_fetch(self::TAGS_PREFIX.$tag);
			...
			$masterTagWrapper = apc_fetch(self::TAGS_MASTER_ID);
			$masterTagList = null;
			...
			foreach ($masterTagList as $tag) {
				$needsUpdate = false;
				$other_tagList = apc_fetch(self::TAGS_PREFIX.$tag);
				if (is_array($other_tagList) ) {
					...
					if ($needsUpdate) {
						...
						if ( count($other_tagList) < 1) {
				    		...
				    		apc_store(self::TAGS_PREFIX.$tag, $other_tagList);
						}
					}
				}
			}
